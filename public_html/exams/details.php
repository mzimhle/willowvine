...
if(count($_POST) > 0) {
	...
	if(isset($_POST['enquiry_email']) && trim($_POST['enquiry_email']) != '') {
		if($examObject->validateEmail(trim($_POST['enquiry_email'])) == '') {
	...
	if(count($errorArray) == 0 && $formValid == true) {
		...
		$data['enquiry_email']			= $examObject->validateEmail(trim($_POST['enquiry_email']));	
		$data['enquiry_message']		= trim($_POST['enquiry_message']);			
		$data['enquiry_reference']		= $examData['exam_code'];
		$data['enquiry_type']				= 'INTERNSHIP';
		
		$ext 			= strtolower($fileObject->file_extention($_FILES['cvdocument']['name']));					
		$filename	= $fileObject->getRandomFileName($examData['exam_code']).'.'.$ext;
		$directory	= $_SERVER['DOCUMENT_ROOT'].'/media/exams/'.$examData['exam_code'].'/application/';
		...
		if(file_put_contents($file, file_get_contents($_FILES['cvdocument']['tmp_name']))) {
			...
			$data['enquiry_file_path']	= '/media/exams/'.$examData['exam_code'].'/application/'.$filename;
			...
			if($success) {
				...
				if($enquiryData) {
					...
					$to[] = array('name' =>$examData['exam_poster_name'], 'email' => $examData['exam_poster_email']);
					
					$enquirysuccess = $enquiryObject->_comm->sendEnquiry('templates/mail/enquiry_exam.html', $enquiryData, 'Willowvine Internship Application: '.$examData['exam_name'], $to);
...
$smarty->display('exams/details.tpl');
?>