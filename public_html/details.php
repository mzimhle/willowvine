...
if(count($_POST) > 0) {
	...
	if(isset($_POST['enquiry_email']) && trim($_POST['enquiry_email']) != '') {
		if($jobObject->validateEmail(trim($_POST['enquiry_email'])) == '') {
	...
	if(count($errorArray) == 0 && $formValid == true) {
		...
		$data['enquiry_email']			= $jobObject->validateEmail(trim($_POST['enquiry_email']));	
		$data['enquiry_message']		= trim($_POST['enquiry_message']);			
		$data['enquiry_reference']		= $jobData['job_code'];
		$data['enquiry_type']				= 'JOB_APPLICATION';
		
		$ext 			= strtolower($fileObject->file_extention($_FILES['cvdocument']['name']));					
		$filename	= $fileObject->getRandomFileName($jobData['job_code']).'.'.$ext;
		$directory	= $_SERVER['DOCUMENT_ROOT'].'/media/jobs/'.$jobData['job_code'].'/application/';
		...
		if(file_put_contents($file, file_get_contents($_FILES['cvdocument']['tmp_name']))) {
			...
			$data['enquiry_file_path']	= '/media/jobs/'.$jobData['job_code'].'/application/'.$filename;
			...
			if($success) {
				...
				if($enquiryData) {
					...
					$to[] = array('name' =>$jobData['job_poster_name'], 'email' => $jobData['job_poster_email']);
					
					$enquirysuccess = $enquiryObject->_comm->sendEnquiry('templates/mail/enquiry_job.html', $enquiryData, 'Willowvine Job Application: '.$jobData['job_title'], $to);
