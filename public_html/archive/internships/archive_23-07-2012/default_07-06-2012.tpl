...
<title>WillowVine | Internships {if isset($sectionData)}in {$sectionData.section_name}{/if} returned {$paginator->totalItemCount|default:'0'} results, page {$page|default:"1"} {if $searchInternship neq ""} while searching for "{$searchInternship}"{/if}</title>
<meta name="description" content="Willowvine internship search for {if isset($sectionData)}{$sectionData.section_name}, {/if}{$searchInternship|trim|default:'any internships'|escape} on willowvine and {$paginator->totalItemCount|default:'0'} internship results, page {$page|default:'1'} {if $searchInternship neq ''} while searching for '{$searchInternship}'{/if}" />
...
<div class="left_content">
		<span class="descr_text">
			Searching for <span class="green_text">"{$searchInternship|default:"all internships"}"</span> which brought back
...
{if $internshipItems|@count gt 0}		
	...
	<p class="pagination fr">
		{if $paginator->current gt 1}
			<a class="prev-page" href="/internships/?page={$paginator->previous}&searchInternship={$searchInternship}">Previous</a>
		{/if}	
			{foreach from=$paginator->pagesInRange item=page}
			{if $page neq $paginator->current}		
			<a class="page-num" href="/internships/?page={$page}&searchInternship={$searchInternship}&searchArea={$searchArea}">{$page}</a>
			{else}
			<strong class="current-page">{$page}</strong>
			{/if}
		{/foreach}
		{if $paginator->current lt $paginator->lastPageInRange}
			<a class="next-page" href="/internships/?page={$paginator->next}&searchInternship={$searchInternship}">Next</a>
		{/if}	
	</p>
	{/if}
	{/capture}	
