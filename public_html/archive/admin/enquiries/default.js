$(document).ready(function(){
	send_filter(1);						
});

var retries = 0;

function fetchGrid(newPage,queryString) {
	// Make sure that we do not attach a NULL to a string
	if (queryString == null) queryString = '';

	retries++;
	if (retries < 4){
		
		$.ajax({
				type: "GET",
				url: "/admin/enquiries/enquiriesHtml.php",
				data: "page="+newPage+queryString,
				dataType: "html",
				timeout: 10000,
				success: function(userItems){
						//show table
						$('#tableContent').html(userItems);
						$('#tableContent').fadeIn('slow');
				},
				error: function(){
					$('#tableContent').html("Try "+retries+" failed, trying "+(retries+1)+" of 3 retries.");
					fetchGrid(newPage);
				}
		});
	}else{
		retries = 0;
	}	
}

function send_filter(page) 
{
	fetchGrid(page,'&search_text~t=' + $('#search_text').val()  + '&user_added~sd=' + $('#start_date').val()  + '&user_added~ed=' + $('#end_date').val() + '&per_page~i=' + $('#per_page').val());
}