...
<div class="left_content">
		<span class="descr_text">
			You have <span class="green_text">"{$paginator->totalItemCount|default:"0"}"</span> shortlisted jobs.
		</span>
		<br>
{if $jobShareCount gt 0}	
<a class="standard_blue_btn fr" href="Javascript:{if isset($userData)}saveShortList();{else}showLogin();{/if}">
...
{if $jobItems|@count gt 0}		
	...
	<p class="pagination fr">
		{if $paginator->current gt 1}
			<a class="prev-page" href="/jobs/shortlist/?page={$paginator->previous}&searchJob={$searchJob}&searchArea={$searchArea}">Previous</a>
		{/if}	
			{foreach from=$paginator->pagesInRange item=page}
			{if $page neq $paginator->current}		
			<a class="page-num" href="/jobs/shortlist/?page={$page}&searchJob={$searchJob}&searchArea={$searchArea}">{$page}</a>
			{else}
			<strong class="current-page">{$page}</strong>
			{/if}
		{/foreach}
		{if $paginator->current lt $paginator->lastPageInRange}
			<a class="next-page" href="/jobs/shortlist/?page={$paginator->next}&searchJob={$searchJob}&searchArea={$searchArea}">Next</a>
		{/if}	
	</p>
	{/if}
	{/capture}	
	...
	<div class="search_item search_odd_{if $smarty.foreach.foo.index is even}blue{else}dark{/if}">
		<div class="title_box">
			<a style="color: #063166;" title="View this job, {$item.job_title|normal_text}'s details" href="/jobs/shortlist/{$item.section_urlName}/details/{$item.job_link}/{$item.job_reference}/?searchArea={$searchArea}&searchJob={$searchJob}&sectionId={$sectionId}&page={$paginator->current}" style="text-decoration: none;">
		...
		<ul>
			<li>
				<a href='javascript:void(0);' onclick="openWindow_fb_share('http://www.facebook.com/sharer/sharer.php?u=http://www.willowvine.co.za/jobs/shortlist/{$item.section_urlName}/details/{$item.job_link}/{$item.job_reference}/');" title="Share this job on facebook: {$item.job_title|normal_text}">
			...
			<li>
				<a class="small_blue_bg_dark_btn fr" title="Share job or career with a friend" alt="Share job or career with a friend" href="Javascript:shortListRemove({$item.job_reference});">
