...
if(isset($userData) && count($userData) > 0) {
	...
	if(is_numeric($shortListObject->remove($userData[0]['jobSeeker_reference']))) {
		...
		if((isset($slsession->jobs) && count($slsession->jobs) > 0)) {
			...
			echo '<span style="color: green;"><b>Your shortList was saved.</b></span>';
			exit;
		}
			echo '<span style="color: green;"><b>Your list has been deleted.</b></span>';
			exit;		
	} else {
		echo '<span style="color: red;"><b>There was an error saving, please try again later.</b></span>';
	}
} else {
	echo '<span style="color: red;"><b>You are not logged in.</b></span>';
}
?>