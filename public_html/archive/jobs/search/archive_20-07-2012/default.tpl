...
{if $jobItems|@count gt 0}		
	...
	<p class="pagination fr">
		{if $paginator->current gt 1}
			<a class="prev-page" href="/jobs/search/{if isset($sectionData)}{$sectionData.section_urlName}/{/if}?page={$paginator->previous}&searchJob={$searchJob}&searchArea={$searchArea}">Previous</a>
		{/if}	
			{foreach from=$paginator->pagesInRange item=page}
			{if $page neq $paginator->current}		
			<a class="page-num" href="/jobs/search/{if isset($sectionData)}{$sectionData.section_urlName}/{/if}?page={$page}&searchJob={$searchJob}&searchArea={$searchArea}">{$page}</a>
			{else}
			<strong class="current-page">{$page}</strong>
			{/if}
		{/foreach}
		{if $paginator->current lt $paginator->lastPageInRange}
			<a class="next-page" href="/jobs/search/{if isset($sectionData)}{$sectionData.section_urlName}/{/if}?page={$paginator->next}&searchJob={$searchJob}&searchArea={$searchArea}">Next</a>
		{/if}	
	</p>
	{/if}
	{/capture}	
