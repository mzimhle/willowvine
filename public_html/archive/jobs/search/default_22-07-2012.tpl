...
<div id="contnav" class="fl">
	<p class="fl">
		...
		<span>|</span> &nbsp; 
		<span>Searching for <span class="green_text">"{$searchJob|default:"All jobs"}"</span> in <span class="green_text">"{$location|trim|default:"South Africa"}"</span>			
	</p>
</div>
...
<div class="left_content">
		<span class="descr_text">
			...
			jobs in <span class="green_text">"{$location|trim|default:"South Africa"}"</span>, which brought back
			<span class="green_text">"{$paginator->totalItemCount|default:"0"}"</span> results you can choose to apply for. {if isset($sectionData)}Jobs for the <span class="green_text">{$sectionData.section_name}</span> category{/if}
			Good luck!
		</span>
{if $jobItems|@count gt 0}		
	...
	<p class="pagination fr">
		{if $paginator->current gt 1}
			<a class="prev-page" href="/jobs/search/{if isset($sectionData)}{$sectionData.section_urlName}/{/if}?page={$paginator->previous}&searchJob={$searchJob}">Previous</a>
		{/if}	
			{foreach from=$paginator->pagesInRange item=page}
			{if $page neq $paginator->current}		
			<a class="page-num" href="/jobs/search/{if isset($sectionData)}{$sectionData.section_urlName}/{/if}?page={$page}&searchJob={$searchJob}">{$page}</a>
			{else}
			<strong class="current-page">{$page}</strong>
			{/if}
		{/foreach}
		{if $paginator->current lt $paginator->lastPageInRange}
			<a class="next-page" href="/jobs/search/{if isset($sectionData)}{$sectionData.section_urlName}/{/if}?page={$paginator->next}&searchJob={$searchJob}">Next</a>
		{/if}	
	</p>
	{/if}
	{/capture}	
	...
	<div class="search_item search_odd_{if $smarty.foreach.foo.index is even}blue{else}dark{/if}">
		<div class="title_box">
			<a style="color: #063166;" title="View this job, {$item.job_title|normal_text}'s details" href="/jobs/search/{$item.section_urlName}/details/{$item.job_link}/{$item.job_reference}/?searchJob={$searchJob}&page={$paginator->current}" style="text-decoration: none;">
