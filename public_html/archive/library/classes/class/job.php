...
class class_job extends Zend_Db_Table_Abstract
{
   //declare table variables
	...
	public function getSearchJob() {
		...
		$select = $this->_db->select()	
					   ->from(array('job' => 'job'), array('DISTINCT(job_title)'))					   
					   ->where('job_active = 1 AND job_deleted = 0')					   
					   ->order('job_title');
		return $this->_db->fetchAll($select);					   
	}
	...
	public function jobCountByArea($province, $jobSearch, $whereArea, $whereType, $whereSection, $limit = 40) {

		$whereAdd = '';
		if($province != '' AND $province != 'South Africa') { 
			$whereAdd .=	"job_area LIKE '%".$province."%'";
		} else {
			$whereAdd .= "job_area != ''";
		}
		
		$whereArea		= $whereArea == '' ? 'job_title != \'\'' : $whereArea;
		$whereType		= $whereType == '' ? 'job_title != \'\'' : $whereType;
		$whereSection	= $whereSection == '' ? 'job_title != \'\'' : $whereSection;
		
		if($jobSearch != '') $whereAdd .=	" AND job_title LIKE '%".$jobSearch."%'";

		$select = $this->_db->select()
						->from(array('job' => 'job'), array('COUNT(pk_job_id) AS job_count', 'job_area', 'TRIM(REPLACE(REPLACE(LOWER(job_area), \', \', \'_\'), \' \', \'_\')) AS job_area_link'))
						->join('areaMap', 'areaMap.areaMap_shortPath = job.job_area', array())
	...
	public function jobCountByType($province, $jobSearch, $whereArea, $whereType, $whereSection) {
		...
		$select = $this->_db->select()
						->from(array('job' => 'job'), array('COUNT(pk_job_id) AS job_count', 'job_type'))
						->join('areaMap', 'areaMap.areaMap_shortPath = job.job_area', array())
						->where('job.job_active = 1 AND job.job_deleted = 0')
						->where($whereAdd)
						->where($whereArea)
	...
	public function getPaginatedjob($where = 'job_title != ""', $order = NULL, $page = 1, $perPage = 10, $listedPages = 10, $scrollingStyle = 'Sliding')
	{
			...
			$select = $this->_db->select()
							->from(array('job' => 'job'))
							->join('section', 'section.pk_section_id = job.fk_section_id', array('section_name', 'section_urlName'))
							->join('areaMap', 'areaMap.areaMap_shortPath = job.job_area', array())
						   ->where($where)						   				  
	...
	{	
		if($search == '') {
			$select = $this->_db->select()
							->from(array('job' => 'job'))
							->join('section', 'section.pk_section_id = job.fk_section_id', array('section_name', 'section_urlName'))
							->join('areaMap', 'areaMap.areaMap_shortPath = job.job_area')					   				  
						   ->where("job_deleted = 0")
						   ->order('job_added desc');
		} else {
			$select = $this->_db->select()
							...
							->join('areaMap', 'areaMap.areaMap_shortPath = job.job_area')					   				  
						   ->where("job_deleted = 0")
						   ->where("lower(concat(job_title, ' ', section_name, ' ', job.job_area)) like lower(?)", "%$search%")
						   ->order('job_added desc');
