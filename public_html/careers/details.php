...
if(count($_POST) > 0) {
	...
	if(isset($_POST['enquiry_email']) && trim($_POST['enquiry_email']) != '') {
		if($careerObject->validateEmail(trim($_POST['enquiry_email'])) == '') {
	...
	if(count($errorArray) == 0 && $formValid == true) {
		...
		$data['enquiry_email']			= $careerObject->validateEmail(trim($_POST['enquiry_email']));	
		$data['enquiry_message']		= trim($_POST['enquiry_message']);			
		$data['enquiry_reference']		= $careerData['career_code'];
		$data['enquiry_type']				= 'JOB_APPLICATION';
		
		$ext 			= strtolower($fileObject->file_extention($_FILES['cvdocument']['name']));					
		$filename	= $fileObject->getRandomFileName($careerData['career_code']).'.'.$ext;
		$directory	= $_SERVER['DOCUMENT_ROOT'].'/media/careers/'.$careerData['career_code'].'/application/';
		...
		if(file_put_contents($file, file_get_contents($_FILES['cvdocument']['tmp_name']))) {
			...
			$data['enquiry_file_path']	= '/media/careers/'.$careerData['career_code'].'/application/'.$filename;
			...
			if($success) {
				...
				$enquiryData = $enquiryObject->getByCode($success, 'JOB_APPLICATION');
				
				if($enquiryData) {
					
					$to = array();
					$to[] = array('name' =>$enquiryData['enquiry_name'], 'email' => $enquiryData['enquiry_email']);
					$to[] = array('name' =>$careerData['career_poster_name'], 'email' => $careerData['career_poster_email']);
					
					$enquirysuccess = $enquiryObject->_comm->sendEnquiry('templates/mail/enquiry_career.html', $enquiryData, 'Willowvine Job Application: '.$careerData['career_title'], $to);
