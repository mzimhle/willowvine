	...
	<div class="row">
    	...
    	<div class="col-xs-12 col-md-5 db-gutter-right">
			...
			<div class="tenderbox eachbox">
				<p style="font-size: 14px !important;">There are <span class="redhighlight">{$jobData.count}</span> jobs in <span class="redhighlight">{$jobData.pages}</span> pages that have been found. {if isset($filters.filters_title)}Searching for <i><span class="redhighlight">"{$filters.filters_title}"</span></i>.{/if}{if isset($filters.category.category_name)}Category being searched for is the <i><span class="redhighlight">"{$filters.category.category_name}</span></i>.{/if}{if isset($filters.type)}Types being searched for <i><span class="redhighlight">"{$filters.type|capitalize}"</span></i>. {/if}{if isset($filters.filters_province)}Province searched in is <i><span class="redhighlight">"{$filters.filters_province|capitalize}"</span></i>.{/if}</p>
				{capture name="pagination"}
				{if $jobData.pages gt 0}
				<div class="pagibox">
					<ul class="pagination pagination-sm">
						{if $pageNumber gt 1 }
						  <li><a href="/jobs/{math equation="c - p" c=$pageNumber p=$one}"><span class="fa fa-chevron-left"></span></a></li>
						  {/if}
						  {section name=foo start=$pageNumber loop=10 step=1}
						  <li {if $smarty.section.foo.iteration eq $pageNumber} class="active" {/if}><a href="/jobs/{$smarty.section.foo.index}">{$smarty.section.foo.iteration}</a></li>
						 {/section}
						  {if $pageNumber lt $jobData.pages}
						  <li><a href="/jobs/{math equation="c + p" c=$pageNumber p=$one}"><span class="fa fa-chevron-right"></span></a></li>
						  {/if}
					</ul>
				</div>
				{/if}
				...
				{foreach from=$jobData.records item=item name=foo}
