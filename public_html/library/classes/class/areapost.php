   ...
   //declare table variables
	...
	public function search($query, $limit = 20) {
		...
		$select = $this->_db->select()
					   ...
					   ->where("lower(areapost.areapost_name) like lower(?)", "%$query%")
					   ->limit($limit)
	...
	public function searchParents($query, $limit = 20) {

		$select = $this->_db->select()->distinct()
						->from(array('areapost' => 'areapost'), array('areapost.areapost_parent as areapost_code', "concat(areapost.areapost_city, ', ',demarcation_name) as areapost_name"))
						->joinLeft('areapostregion', 'areapostregion.areapostregion_code = areapost.areapostregion_code', array())
					   ->where("concat(areapost.areapost_city, ', ',demarcation_name) like lower(?)", "%$query%")
					   ->limit($limit)
					   ->order("LOCATE('$query', concat(areapost.areapost_city, ', ',demarcation_name))");
