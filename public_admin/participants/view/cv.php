...
if (!empty($_GET['code']) && $_GET['code'] != '') {
	...
	if($participantData) {
		...
		$cvData = $cvObject->getByParticipant($code);
...
if(isset($_FILES['documentfile'])) {
	...
	if(count($errorArray) == 0 && $formValid == true) {
		...
		$filename	= $fileObject->getRandomFileName($participantData['participant_code']).'.'.$ext;
		$directory	= $_SERVER['DOCUMENT_ROOT'].'/media/participants/'.$participantData['participant_code'].'/cv/';
		...
		if(file_put_contents($file, file_get_contents($_FILES['documentfile']['tmp_name']))) {
			...
			$data['cv_path']		= '/media/participants/'.$participantData['participant_code'].'/cv/'.$filename;
			...
			if($success) {
				header('Location: /participants/view/cv.php?code='.$code);
				exit;
			} else {
...
$smarty->display('participants/view/cv.tpl');
