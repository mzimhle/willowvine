...
if(isset($_GET['page']) && (int)trim($_GET['page']) > 0 && isset($_GET['cat']) && (int)trim($_GET['cat']) > 0 ) {
	...
	$page	= (int)$_GET['page'] == 1 ? 1 : ($page*20) + 1;
	...
	while(($item = str_get_html($results)->find('.goldborder tr', $counter)->innertext) != NULL) {		
		...
		$data['job_title'] 				= str_get_html(str_get_html($item)->find('.karen ', 2)->innertext)->find('strong', 0)->innertext; 
		...
		$data['job_page']	= str_get_html($pageContent)->find('.karen', 0)->innertext; 
