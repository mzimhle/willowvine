...
function showShareInternship(internshipDetails, id) {
	$('#lightboxShareInternshipFormDiv').show();
	$('#internship_details').html(internshipDetails);
	$('#internship_id').val(id);
	$('#lightbox_InternshipShare').modal({overlayClose:true,onClose:function (dialog){$.modal.close();}});}

/* Send share details. */
function submitShareForm(job, name, email, friend_name ,friend_email) {
	$('#share_output').html('<span style="color: green;">Please wait...</span>');
	$.ajax({
			type: "GET",
...
/* Send share details. */
function submitInternshipShareForm(id, name, email, friend_name ,friend_email) {
	$('#internship_output').html('<span style="color: green;">Please wait...</span>');
	$.ajax({
			type: "GET",
			url: "/includes/share_internship_with_friend.php",
			data: "name="+name+"&email="+email+"&friend_email="+friend_email+"&id="+id+"&friend_name="+friend_name,
			dataType: "html",
			timeout: 10000,
			success: function(data){
				$('#internship_output').html(data);
			},
			error: function(){
				$('#internship_output').html('Share Error: Please try again.');
			}
	});
}
